...
const Invoice = () => {
  ...
  const inVoiceData = record.selectTests.map((test) => ({
    ...
    min: testDB.find((dbTest) => dbTest.id === test.testId)?.min || " ",
    max: testDB.find((dbTest) => dbTest.id === test.testId)?.max || " ",
    unit: testDB.find((dbTest) => dbTest.id === test.testId)?.unit || "no data",
  }));
            ...
            <TableBody>
              {inVoiceData.map((item) => (
                <TableRow key={item.testID}>
                  ...
                  <TableCell>{item.testresults || ((item.max + item.min) / 2) - (item.max - item.min) / 4}</TableCell>
