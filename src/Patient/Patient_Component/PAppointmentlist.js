...
export default function StickyHeadTable() {
  ...
  useEffect(() => {
    axios.get('https://healthlabbackend.vercel.app/api/appointments')
      .then(response => {
  ...
  const handleLinkClick = (event, value, state) => {
    ...
    if (state === 'Doctor_approved' && jwtDecode(localStorage.getItem("myToken")).role === 'PATIENT') {
      axios.post('https://healthlabbackend.vercel.app/api/updateappointment', { id: value, patientView: 'viewed' })
        .then(response => {
